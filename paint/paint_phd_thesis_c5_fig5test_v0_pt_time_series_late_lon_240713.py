...
def main():
    ...
    tfile = xr.open_dataset("/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/composite_ERA5/temperature_composite_abnormal.nc")
    tfile_1000 = tfile.sel(lev=925, lat=slice(5,0))
    tfile_850  = tfile.sel(lev=600, lat=slice(5, 0))

    sf_series1 = np.nanmean(np.nanmean(tfile_850['t_early'].data[20:30], axis=0), axis=0) * ((1000/600)**0.286)
    sf_series2 = np.nanmean(np.nanmean(tfile_1000['t_early'].data[20:30], axis=0), axis=0)*((1000/925)**0.286) - sf_series1

    print(sf_series2[60:90])
