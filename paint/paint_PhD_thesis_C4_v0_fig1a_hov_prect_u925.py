...
#    f1  =  xr.open_dataset("/home/sun/data/climate_merra2_u_925.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
    ...
    cf   = ax2.contourf(f0.lon.values, f0.time.values, 1.05*trmm.data,clevs_p, cmap=newcmp, extend='both')
    cs   = ax2.contour(f0.lon.values,  f0.time.values,1.05*trmm.data, clevs_p, colors='k', linewidths=1.5)
