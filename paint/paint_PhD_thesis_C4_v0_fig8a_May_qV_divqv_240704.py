...
# =========== File Information ==============
f_ctl = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_control_climate_atmosphere.nc").sel(lev=slice(1000, 850))
f_ind = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_indian_climate_atmosphere3.nc").sel(lev=slice(1000, 850))
...
diff_div = f_qdiv['vert_integral_control'].data - f_qdiv['vert_integral_indian'].data
#print(diff_div.shape)
for latt in range(len(f_ctl.lat.data)):
    for lonn in range(len(f_ctl.lon.data)):
        if f_ctl.lat.data[latt] > 4 and f_ctl.lat.data[latt] < 25 and f_ctl.lon.data[lonn] > 67 and f_ctl.lon.data[lonn] < 92 and mask_file_interp.lsm.data[0, latt, lonn] > 0.3:
            diff_div[latt, lonn] = 999

diff_div = np.ma.masked_where(diff_div==999, diff_div)
...
# 绘制降水填色图
level0 = np.linspace(-3, 3, 11)
im  =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data,diff_div*1e6,level0,cmap=cmap,alpha=1,extend='both')
...
# 绘制矢量图
#q  =  ax.quiver(f_ctl.lon.data,f_ctl.lat.data, qu*1e9, qv*1e9, 
...
# 保存图片
save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c4_fig8b_May_moisture_transportation.pdf")
