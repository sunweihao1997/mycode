...
# =========== File Information ==============
f_ctl = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_control_climate_atmosphere.nc").sel(lev=925)
f_ind = xr.open_dataset("/home/sun/data/model_data/climate/b1850_exp/b1850_inch_climate_atmosphere.nc").sel(lev=925)
...
ts    = np.average(f_ctl['T'].data[start0:end0], axis=0)    - np.average(f_ind['T'].data[start0:end0], axis=0)
#pr    = np.average(f_ind['PRECT'].data[start0:end0], axis=0)
#u     = np.average(f_ind['U'].data[start0:end0], axis=0)
#v     = np.average(f_ind['V'].data[start0:end0], axis=0)
...
# 绘制降水填色图
im  =  ax.contourf(f_ctl.lon.data,f_ctl.lat.data,pr*86400000,np.linspace(-2.5,2.5,11),cmap='coolwarm',alpha=1,extend='both')
...
# 保存图片  
save_fig(path_out="/home/sun/paint/phd/",file_out="phd_c4_fig9_Apr_precipitation_inch.pdf")
