...
def paint_picture(lon, lat, u, v, psl):
    # 绘制图像
    proj    =  ccrs.PlateCarree()
    fig,ax   =  plt.subplots(figsize=(13,8.5),subplot_kw=dict(projection=ccrs.PlateCarree()))
    ...
    im  =  ax.contourf(lon,lat,psl,np.linspace(-6,6,13),cmap='coolwarm',alpha=1,extend='both')


    # 绘制矢量图
    q  =  ax.quiver(lon, lat, u, v, 
                regrid_shape=12, angles='uv',   # regrid_shape这个参数越小，是两门就越稀疏
                scale_units='xy', scale=1.75,        # scale是参考矢量，所以取得越大画出来的箭头就越短
                units='xy', width=0.3,
                transform=proj,
    ...
    # 加矢量图图例
    add_vector_legend(ax=ax,q=q, speed=10)
    ...
    # 保存图片
    save_fig(path_out="/home/sun/paint/monthly_variable/regression/",file_out="March_qV_to_LSTC_index.png")

def main():
    f1 = xr.open_dataset('/home/sun/data/monsoon_onset_anomaly_analysis/ERA5_data_monsoon_onset/regression/ERA5_regression_1000to700_avg_moisture_transportation_to_LSTC.nc')
    #print(f1)

    print(np.nanmean(f1['rc_div'].data))
    paint_picture(lon=f1.lon,lat=f1.lat,u=f1['rc_u'].data*-1e4, v=f1['rc_v'].data*-1e4, psl=f1['rc_div'].data*-1e10)
